...
make_pidfile( const char* fpath, pid_t pid, FILE* log )
{
    ...
    errno = 0;
    do {
        fd = open( fpath, O_CREAT | O_WRONLY, fmode );
        if( -1 == fd ) {
        ...
        n = snprintf( buf, sizeof(buf) - 1, "%lu", (unsigned long)pid );
        if( n < 0 ) {
...
int would_block(int err) { return (EAGAIN == err) || (EWOULDBLOCK == err); }
...
write_buf( int fd, const char* data, const ssize_t len, FILE* log )
{
    ...
    for( n = 0; errno = 0, n < len ; ) {
        ...
        if( nwr <= 0 ) {
            err = errno;
            if( EINTR == err ) {
                TRACE( (void)tmfprintf( log, "%s interrupted\n", __func__ ) );
                continue;
            } else {
                if( would_block(err) ) error = IO_BLK;
                break;
            }
        }
        n += nwr;
        if( nwr != len ) {
            if( NULL != log ) {
                TRACE( (void)tmfprintf( log, "Fragment written %s[%ld:%ld]/[%ld] bytes\n", (len > n ? "P" : "F"), (long)nwr, (long)n, (long)len ) );
            }
        }
    }
    if( nwr <= 0 ) {
...
int
sizecheck( const char* msg, ssize_t expct, ssize_t len, FILE* log, const char* func )
{
    assert( msg && log && func );
    if( expct == len ) return 0;
    (void) (msg && log && func);
    TRACE( (void)tmfprintf( log, "%s: %s - read only [%ld] bytes out of [%ld]\n", func, msg, (long)len, (long)expct ) );
    return 1;
}

int
buf_overrun( const char* buf, size_t buflen, size_t offset, size_t dlen, FILE* log )
{
    const char* tgt = buf + offset + dlen;
    if( tgt > (buf + buflen) ) {
        if( NULL != log ) {
            TRACE( (void)tmfprintf( log, "+++ BUFFER OVERRUN at: buf=[%p], size=[%lu] - intending to access [%p]: offset=[%lu], data_length=[%lu]\n", buf, (u_long)buflen, tgt, (u_long)offset, (u_long)dlen) );
        }
        return 1;
    }
    return 0;
...
int
mk_tvstamp( const struct timeval* tv, char* buf, size_t* len, int32_t flags )
{
    ...
    assert( tv && buf && len );
    clock = tv->tv_sec;
    if (flags & TVSTAMP_GMT) {
        if (0 != gmtime_r(&clock, &src_tm)) {
            perror("gmtime_r");
            return errno;
        }
    } else {
        if (0 != localtime_r(&clock, &src_tm)) {
            perror("localtime_r");
            return errno;
        }
    }
...
void
printcmdln( FILE* stream, const char* msg, int argc, char* const argv[] )
{
    int i = 0;
    assert( stream );
    if( msg )
...
a2time( const char* str, time_t* t, time_t from )
{
    ...
    i_hour  = i--;
    i_day   = i--;
    if( 0 != localtime_r( &now, &stm ) ) {
        return errno;
    }
    if( !is_offset ) {
        if( i_hour >= 0 ) stm.tm_hour = field[ i_hour ];
        if( i_day >= 0  ) stm.tm_mday = field[ i_day ];
...
#define MAX_SZLEN 64
    ...
    if( '\0' != *p ) {
        if( 0 == strncasecmp( p, "Kb", MAX_SZLEN ) || 0 == strncasecmp( p, "K", MAX_SZLEN ))
            mult = 1024;
        else if( 0 == strncasecmp( p, "Mb", MAX_SZLEN ) || 0 == strncasecmp( p, "M", MAX_SZLEN ) )
            mult = (1024 * 1024);
        else if( 0 == strncasecmp( p, "Gb", MAX_SZLEN ) || 0 == strncasecmp( p, "G", MAX_SZLEN ) )
            mult = (1024 * 1024 * 1024);
        else
            return ERR_BADMULT;
        numsz = p - str;
...
Zasctime( const struct tm* tm )
{
    size_t n = 0;
#define ASCBUF_SZ 64
    static char buf[ ASCBUF_SZ ];
    buf[0] = '\0';
    if (0 != asctime_r( tm, buf )) return NULL;
    buf[ ASCBUF_SZ - 1 ] = '\0';
    n = strlen( buf );
    if( (n > (size_t)1) && ('\n' == buf[ n - 1 ]) ) {
        buf[ n - 1 ] = '\0';
...
void
check_fragments( const char* action, ssize_t total, ssize_t n_was, ssize_t n_is, ssize_t delta, FILE* log )
{
    if( NULL == action ) return;
    if( (n_is < (n_was - delta)) || (n_is > (n_was + delta)) ) {
        (void)tmfprintf( log, "%s [%ld] bytes out of [%ld], last=[%ld]\n", action, (long)n_is, (long)total, (long)n_was);
    }
}
...
get_pidstr( int reset, const char* pfx )
{
    ...
    if( 1 == reset || (0 == pid) ) {
        pid = getpid();
        if (pfx) {
            (void) snprintf (pidstr, sizeof(pidstr)-1, "%s(%lu)", pfx, (unsigned long)pid);
        } else {
            (void) snprintf (pidstr, sizeof(pidstr)-1, "%lu", (unsigned long)pid );
        }
